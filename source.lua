...
local function GetFullPathOfAnInstance(instance)
    if lastKnownPaths[instance] then
        return lastKnownPaths[instance] -- Возвращаем последний известный путь
    end
    ...
    if not instance.Parent and instance ~= game then
        return lastKnownPaths[instance] or (head .. " --[[ PARENTED TO NIL OR DESTROYED ]]")
    end
    ...
    local path = GetFullPathOfAnInstance(instance.Parent) .. head
    lastKnownPaths[instance] = path -- Сохраняем путь
    return path
end
